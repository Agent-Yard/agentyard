...
from agentyard_extension_sdk.common.canonical_json import sha256_digest


CORE_CHANNEL_GATEWAY_REGISTRATION_ID = "core-channel-gateway"
CORE_AGENT_RUNTIME_REGISTRATION_ID = "core-agent-runtime"
CHANNEL_GATEWAY_BASE_URL_ENV = "AGENTYARD_CHANNEL_GATEWAY_BASE_URL"
AGENT_RUNTIME_BASE_URL_ENV = "AGENTYARD_AGENT_RUNTIME_BASE_URL"
...
def _operator_registrations(yaml_text: str | None, environment: EnvironmentResolver) -> tuple[ExtensionRegistration, ...]:
    ...
    agentyard = document.get("agentyard")
    if agentyard is None:
        services = []
    else:
        _require_mapping(agentyard, "agentyard")
        extensions = agentyard.get("extensions")
        if extensions is None:
            services = []
        else:
            _require_mapping(extensions, "agentyard.extensions")
            services = extensions.get("services")
            if services is None:
                services = []
    if not isinstance(services, list):
        raise _invalid("agentyard.extensions.services must be a list")
