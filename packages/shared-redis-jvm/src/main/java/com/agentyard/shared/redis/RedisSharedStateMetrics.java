...
public class RedisSharedStateMetrics {
    ...
    public void incrementLockAcquireSuccess() {
        counter("agentyard.shared_state.lock.acquire.success").increment();
    }

    public void incrementLockAcquireFailure() {
        counter("agentyard.shared_state.lock.acquire.failure").increment();
    }

    public void incrementIdempotencyCompletedHit() {
        counter("agentyard.shared_state.idempotency.completed.hit").increment();
    }

    public void incrementIdempotencyReserved() {
        counter("agentyard.shared_state.idempotency.reserved").increment();
    }

    public void incrementIdempotencyCompletedWrite() {
        counter("agentyard.shared_state.idempotency.completed.write").increment();
    }

    public void incrementIdempotencyInProgressConflict() {
        counter("agentyard.shared_state.idempotency.in_progress.conflict").increment();
    }

    public void incrementPubSubPublished(String channel) {
        taggedCounter("agentyard.shared_state.redis.pubsub.published", channel).increment();
    }

    public void incrementPubSubReceived(String channel) {
        taggedCounter("agentyard.shared_state.redis.pubsub.received", channel).increment();
    }

    public void incrementInvalidationPublished(String domain) {
        taggedCounter("agentyard.shared_state.cache.invalidation.published", domain).increment();
    }

    public void incrementInvalidationReceived(String domain) {
        taggedCounter("agentyard.shared_state.cache.invalidation.received", domain).increment();
    }

    public void recordInvalidationSubscriberLag(String domain, Duration lag) {
        taggedTimer("agentyard.shared_state.cache.invalidation.subscriber.lag", domain).record(
