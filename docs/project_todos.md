...
- API：`spring-session-data-redis` 接入 + `SessionRedisConfiguration`；OIDC authorization request 按 `state` 进入 Redis；`RedisIdempotencyService`、`RedisInvalidationBus`、`SharedStateInvalidationSubscriber` 收口
