...
- Tool v1 内置 `SIMPLE_HTTP`、`BUSINESS_CODE_SECRET_HTTP`、`MCP` connector，执行仍在 `agent-runtime` 内；`SIMPLE_HTTP` 可选通过 Integration Account 注入 Bearer header，复杂厂商协议后续通过 connector registry 扩展
