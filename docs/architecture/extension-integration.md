# Lynxus Extension 接入开发指导

本文件面向需要基于 Lynxus 平台开发独立 extension service 的开发者，给出接入边界、协议契约、运行链路与开发约束。读者不需要熟悉 Lynxus 内部代码；只需要构建一个独立部署的 HTTP 服务，按本文档实现 manifest 与若干 endpoint，再请部署方把服务注册到 Lynxus。

协议事实源是 `@lynxus/extension-protocol` 包（OpenAPI、JSON Schema、examples、contract fixtures）。本文给出"实现一个 extension service 需要落地什么"的行动级指导，不重复协议字段定义。当文档与协议事实源出现不一致时，以协议事实源为准。

## 1. Extension 在系统中的位置

Lynxus 把外部接入面收敛为两类 descriptor：

- `CHANNEL_PROVIDER`：把第三方 IM / 工单 / 客服等会话渠道接入会话运行时，承担入站事件归一化、出站投递、定时拉取等职责
- `TOOL_CONNECTOR`：把第三方业务系统的能力暴露成 Tool operation，由 Lynxus agent 推理时调用
...
- 当声明 `CHANNEL_PROVIDER` 时，订阅由 Lynxus 提供的 outbound frame stream 与 ACK 接口（详见 §6）

protocol 常量与 header 名以 SDK 常量为准：

- JVM：`com.lynxus.extension.sdk.protocol.LynxusExtensionProtocol`、`LynxusExtensionHeaders`
- Python：`lynxus_extension_sdk.protocol`
...
Extension service 必须由部署方在 Lynxus 平台的 registration YAML 中声明（路径由部署侧 `LYNXUS_EXTENSION_REGISTRATION_FILE` 环境变量指定）。Extension 开发者需要把以下信息提供给部署方：
  ...
  extensions:
    services:
      ...
      - registrationId: acme-channel-provider
        baseUrl: http://127.0.0.1:18101/lynxus
        exposes:
          channelProviderTypes:
...
- `registrationId` 不能以 `core-` 开头；该前缀保留给 Lynxus 自身的内置服务
...
平台调用 extension 的所有请求都会携带 `Authorization: Bearer <token>`。Extension service 必须校验该 token；token 由部署方在平台与 extension 两侧通过同一份 secret 配置（约定环境变量名 `LYNXUS_INTERNAL_AUTH_TOKEN`）。SDK 当前未内置 server filter，extension 项目根据自身 web 框架实现校验即可。

请求头分级：

| 调用类型 | 必填头 |
|----------|--------|
| Service-level（GET `/extension/manifest`、`/extension/health`） | `Authorization`，可选 `X-Lynxus-Extension-Registration-Id` |
| Descriptor-level（tool invoke、runJob、outbound 订阅/流/ACK） | `Authorization`、`X-Lynxus-Extension-Registration-Id`、`X-Lynxus-Extension-Descriptor-Type`（`TOOL_CONNECTOR` 或 `CHANNEL_PROVIDER`）、`X-Lynxus-Extension-Descriptor-Id`、`X-Lynxus-Trace-Id`、`X-Lynxus-Request-Id`、`Idempotency-Key` |
| Credential lifecycle | `Authorization`、`X-Lynxus-Trace-Id`、`X-Lynxus-Request-Id` |
...
Extension 把外部渠道事件归一化为 `NormalizedChannelInboundEvent`（schema 见 OpenAPI），通过 `POST <lynxus-base>/internal/channel-events/normalized` 提交给平台。基址由部署方告知 extension（一般是 Lynxus channel gateway 服务的地址）。
...
Lynxus agent runtime 在执行推理时，对每个 Tool 调用按 `connectorType` 路由到 manifest 声明的 `endpoints.invoke`。
...
Credential lifecycle 是 control-plane 操作，由 Lynxus 控制面在 Web/admin 触发 Integration Account 操作时调用：
...
- JVM：`packages/extension-sdk-jvm`，关键入口：
  - `LynxusExtensionProtocol`、`LynxusExtensionHeaders`：协议常量
  - `ManifestValidator` / `ManifestValidationResult`：manifest schema 验证
  - `ExtensionRegistrationLoader`：YAML 解析与 baseUrl 归一化（部署方使用）
  - `LynxusCanonicalJson`、`DescriptorDefinitionDigests`：稳定摘要
  - `LynxusExtensionHttp`：URL 拼接与 header 构造帮助
- Python：`packages/extension-sdk-python`，`lynxus_extension_sdk.protocol` 暴露同名常量与帮助函数；`lynxus_extension_sdk.validation.manifest`、`lynxus_extension_sdk.tool` / `channel`、`lynxus_extension_sdk.common.canonical_json` 等
...
`@lynxus/extension-protocol` 是版本化协议；当前 `extensionApiVersion = 1`。Lynxus 不承诺在协议版本之间向后兼容，也不会为单个 extension 维护双协议并存或字段宽容默认值。

Extension 项目的应对方式：

- 在 manifest 的 `coreMinVersion` / `coreMaxVersion` 中声明实际兼容的 Lynxus 版本范围
- 升级 Lynxus 时同步升级 `@lynxus/extension-protocol` 与对应 SDK 版本，按新版协议事实源回归本文 §13 的全部校验项
- 协议字段层面的不兼容变更（新增/删除字段、收紧约束）随 `extensionApiVersion` 主版本变更披露；extension 项目必须在主版本变更后重新发布

需要 Lynxus 平台扩展协议本身（新增 endpoint、新增 descriptor 类型、改变 frame 语义等）时，请通过平台维护方提出协议变更请求，不要在 extension 端通过 manifest 自定义字段绕过。
