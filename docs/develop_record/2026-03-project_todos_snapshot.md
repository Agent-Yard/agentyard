...
| **2.2 已完成（首版）** | **删除前影响预览** | catalog_todo §2 | 已完成首版：`domain / scenario / assistant / agent / resource / knowledge_base` 六类一级对象删除已统一接入预览弹窗；删除前可看到阻断对象、受影响快照/编排/绑定关系，以及自动级联回收清单。资源版本与知识发布版本删除预览留待后续治理。 |
| **2.3 已完成** | **知识库真实导入与检索链路补齐** | catalog_todo §4 | 已完成：文件上传与 URL 导入已统一进入正式导入任务模型；知识服务补齐了 `source object -> import job -> document -> snapshot` 四层状态机、进度追踪、失败原因与手动重试；Web 知识库工作台已支持后台轮询、失败重试和基于 `READY` snapshot 的检索验证。当前边界保持“手动重试 + 手动创建快照/发布”，不做自动发布与召回评估。 |
| **2.4 已完成（第一阶段）** | **身份与权限体系（OIDC/IAM）** | 规划文档 | 已完成第一阶段：API 已引入正式 `platform_user / platform_user_role_binding`，`/auth/session` 改为从本地 bootstrap admin 会话读取，不再返回硬编码开发用户；同时预留外部身份校验与本地用户 provisioning 服务抽象。当前尚未启用真实登录流、OIDC 跳转、请求级鉴权与细粒度 RBAC。 |
