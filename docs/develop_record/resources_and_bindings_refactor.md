...
## Summary
当前这轮改造已经把资源模型从 `Skill + MCP` 双概念收口到统一 `Tool`，并同步收敛发布冻结与运行时生效规则，重点解决 5 件事：

- 明确资源关系语义：默认引用、覆盖引用、工具启用、发布冻结分别是什么
...
- 明确资源类型定位：
  ...
  - `TOOL`：只作为 agent 可调用工具资源，provider 决定其 `HTTP / MCP` 实现方式
- 控制面不再暴露“工具版本固定”概念；agent 草稿态只维护 Tool 启用，发布时统一冻结当前生效版本
- 资源归属规则收紧：
...
- `Catalog` 草稿态 DTO / 前端 types 调整：
  ...
  - `RagPolicy` 删除 `topK`
  - 智能体只维护 `toolResourceIds`，不再维护手工版本 pin
...
- 发布快照契约改造为“显式版本解析”：
  ...
  - 工具快照保存启用工具在发布时冻结的 `resourceVersionId[]`
- 发布时校验规则固定为：
  - 非工具资源在发布时自动冻结“当前生效版本”
  - 工具资源在发布时自动冻结“当前生效版本”
  - 同一智能体内如果工具资源没有生效版本，返回明确错误
  - 旧的“手工版本绑定”能力全部移除；发布只认当前生效版本
...
- `ResourceLibraryPage` 改成“引用分析”视图，不只展示显式工具版本固定，要能看到：
  ...
  - 工具启用
  - 发布快照冻结引用
- `ResourceCreatePage` 和架构文档统一成 4 类资源蓝图：知识库、Tool、LLM、Prompt
- 旧 JSON catalog 与旧 pin DTO 不再兼容读取，统一按新模型重建 seed/demo

## Test Plan
- Catalog 服务单测：
  - 资源归属校验
  - 资源中心引用分析覆盖默认引用、覆盖引用、工具启用、发布冻结
  - 删除阻断原因与引用分析结果一致
  - 发布时工具无生效版本会失败
...
- 回归场景：
  - 现有 seed 的 FAQ / 售后 Tool / 人工 Tool 闭环保持可运行
