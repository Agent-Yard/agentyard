...
class ChannelGatewayAsyncConfigurationTest {
    ...
    void createsNamedLifecycleManagedInboundExecutorWithConfigurableBounds() {
        contextRunner
            .withPropertyValues(
                "agentyard.channel-gateway.async.inbound-session-dispatch.core-size=3",
                "agentyard.channel-gateway.async.inbound-session-dispatch.max-size=11",
                "agentyard.channel-gateway.async.inbound-session-dispatch.queue-capacity=250",
                "agentyard.channel-gateway.async.inbound-session-dispatch.keep-alive-seconds=45"
            )
            .run(context -> {
