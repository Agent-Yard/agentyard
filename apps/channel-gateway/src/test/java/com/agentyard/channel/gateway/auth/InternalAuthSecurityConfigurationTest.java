package com.agentyard.channel.gateway.auth;
...
import com.agentyard.channel.gateway.channel.ChannelAdminService;
import com.agentyard.channel.gateway.channel.ChannelOutboundExtensionAckService;
import com.agentyard.channel.gateway.channel.ChannelOutboundExtensionController;
import com.agentyard.channel.gateway.channel.ChannelOutboundExtensionStreamService;
import com.agentyard.channel.gateway.channel.ChannelOutboundExtensionSubscriptionService;
import com.agentyard.channel.gateway.channel.ChannelInboundSessionDispatcher;
import com.agentyard.channel.gateway.channel.InternalNormalizedChannelEventController;
import com.agentyard.channel.gateway.channel.InternalNormalizedChannelTurnController;
import com.agentyard.channel.gateway.channel.InternalChannelAdminController;
import com.agentyard.channel.gateway.channel.NormalizedChannelEventIngestService;
import com.agentyard.channel.gateway.channel.NormalizedChannelTurnIngestService;
import com.agentyard.channel.gateway.connector.feishu.FeishuWebhookController;
import com.agentyard.channel.gateway.connector.feishu.FeishuWebhookService;
import com.agentyard.channel.gateway.extension.ChannelGatewayDescriptorProvider;
import com.agentyard.channel.gateway.extension.ExtensionManifestController;
...
class InternalAuthSecurityConfigurationTest {
    ...
    void shouldAllowExtensionOutboundBootstrapWithValidBearerToken() throws Exception {
        try (AnnotationConfigApplicationContext context = createAuthorizedContext()) {
            ...
            mockMvc.perform(get("/extension/channel/outbound-frame-subscriptions")
                    ...
                    .header("X-AgentYard-Extension-Registration-Id", "acme-channel-provider")
                    .header("X-AgentYard-Extension-Descriptor-Type", "CHANNEL_PROVIDER")
                    .header("X-AgentYard-Extension-Descriptor-Id", "enterprise.acme.im"))
                .andExpect(status().isOk())
    ...
    private AnnotationConfigApplicationContext createAuthorizedContext() {
        ...
        context.getEnvironment().getPropertySources().addFirst(new MapPropertySource("test", java.util.Map.of(
            "agentyard.internal-auth.token", "test-internal-token"
        )));
