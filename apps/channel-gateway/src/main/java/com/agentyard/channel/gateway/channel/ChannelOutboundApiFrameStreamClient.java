...
class ChannelOutboundApiFrameStreamClient {
    ...
    void stream(StreamRequest request, StreamHandler handler) throws Exception {
        HttpRequest.Builder builder = HttpRequest.newBuilder()
            ...
            .header("X-AgentYard-Max-Final-Replay-Frames", Integer.toString(request.maxFinalReplayFrames()));
        if (hasText(request.lastEventId())) {
            builder.header("Last-Event-ID", request.lastEventId());
        }
        if (request.lastAckedFinalSequence() != null) {
            builder.header("X-AgentYard-Last-Acked-Final-Sequence", Long.toString(request.lastAckedFinalSequence()));
        }
        if (hasText(request.lastAckedSessionId())) {
            builder.header("X-AgentYard-Last-Acked-Session-Id", request.lastAckedSessionId());
        }
        if (hasText(request.lastAckedSessionMessageId())) {
            builder.header("X-AgentYard-Last-Acked-Session-Message-Id", request.lastAckedSessionMessageId());
        }
