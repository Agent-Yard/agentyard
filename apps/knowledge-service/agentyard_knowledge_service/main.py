...
DEFAULT_DATABASE_URL = "postgresql+psycopg://agentyard:agentyard@127.0.0.1:5432/agentyard_knowledge"


DATABASE_URL = os.getenv("AGENTYARD_KNOWLEDGE_DATABASE_URL", DEFAULT_DATABASE_URL)
STORAGE_SETTINGS = load_storage_settings(DEFAULT_STORAGE_ROOT)
URL_IMPORT_TIMEOUT_SECONDS = float(os.getenv("AGENTYARD_KNOWLEDGE_URL_IMPORT_TIMEOUT_SECONDS", "15"))
URL_IMPORT_USER_AGENT = os.getenv(
    "AGENTYARD_KNOWLEDGE_URL_IMPORT_USER_AGENT",
    "AgentYardKnowledgeService/2.0",
)
EMBEDDING_BASE_URL = os.getenv("AGENTYARD_KNOWLEDGE_EMBEDDING_BASE_URL", "").rstrip("/")
EMBEDDING_MODEL = os.getenv("AGENTYARD_KNOWLEDGE_EMBEDDING_MODEL", "").strip()
EMBEDDING_API_KEY = os.getenv("AGENTYARD_KNOWLEDGE_EMBEDDING_API_KEY", "").strip()
EMBEDDING_DIMENSIONS = int(os.getenv("AGENTYARD_KNOWLEDGE_EMBEDDING_DIMENSIONS", "1024"))
EMBEDDING_BATCH_SIZE = max(1, int(os.getenv("AGENTYARD_KNOWLEDGE_EMBEDDING_BATCH_SIZE", "16")))
EMBEDDING_TIMEOUT_SECONDS = float(os.getenv("AGENTYARD_KNOWLEDGE_EMBEDDING_TIMEOUT_SECONDS", "15"))
DEFAULT_SNAPSHOT_RETRIEVAL_MODE = os.getenv("AGENTYARD_KNOWLEDGE_DEFAULT_RETRIEVAL_MODE", "HYBRID").upper()
DEFAULT_SNAPSHOT_RETRIEVAL_BACKEND = "PGVECTOR"
INSTANCE_ID = (os.getenv("AGENTYARD_INSTANCE_ID") or "agentyard-knowledge-service").strip() or "agentyard-knowledge-service"

engine = create_engine(DATABASE_URL, future=True)
SessionLocal = sessionmaker(bind=engine, autoflush=False, autocommit=False, expire_on_commit=False)
configure_structured_logging("agentyard-knowledge-service", "AGENTYARD_KNOWLEDGE_SERVICE_LOG_LEVEL")
...
def internal_auth_token() -> str:
    token = os.getenv("AGENTYARD_INTERNAL_AUTH_TOKEN", "").strip()
    if not token:
        raise RuntimeError("AGENTYARD_INTERNAL_AUTH_TOKEN must be configured")
    return token
...
class EmbeddingClient:
    ...
    def validate_configuration(self) -> None:
        missing = [
            name
            for name, value in {
                "AGENTYARD_KNOWLEDGE_EMBEDDING_BASE_URL": self.base_url,
                "AGENTYARD_KNOWLEDGE_EMBEDDING_MODEL": self.model,
                "AGENTYARD_KNOWLEDGE_EMBEDDING_API_KEY": self.api_key,
            }.items()
            if not value
        ]
        if self.dimensions <= 0:
            missing.append("AGENTYARD_KNOWLEDGE_EMBEDDING_DIMENSIONS")
        if missing:
...
app = FastAPI(
    title="AgentYard Knowledge Service",
    version="2.0.0",
    lifespan=lifespan,
)
...
async def healthz() -> JSONResponse:
    ...
    report = await build_readiness_report(
        service_name="agentyard-knowledge-service",
