...
def load_storage_settings(default_storage_root: Path) -> ObjectStorageSettings:
    mode = os.getenv("AGENTYARD_OBJECT_STORAGE_MODE", "filesystem").strip().lower()
    if mode not in {"filesystem", "object-storage"}:
        raise ValueError(f"unsupported object storage mode: {mode}")

    storage_root = Path(os.getenv("AGENTYARD_OBJECT_STORAGE_ROOT", str(default_storage_root)))
    provider = os.getenv("AGENTYARD_OBJECT_STORAGE_PROVIDER", "minio").strip().lower()
    if provider not in {"minio", "s3"}:
        raise ValueError(f"unsupported object storage provider: {provider}")

    endpoint = os.getenv("AGENTYARD_OBJECT_STORAGE_ENDPOINT", "").strip()
    if not endpoint and provider == "minio":
        endpoint = "http://127.0.0.1:9000"
    if mode == "object-storage" and not endpoint:
        raise ValueError("AGENTYARD_OBJECT_STORAGE_ENDPOINT is required when object storage is enabled")
    endpoint_host, secure = _parse_endpoint(endpoint) if endpoint else ("", False)

    region = os.getenv("AGENTYARD_OBJECT_STORAGE_REGION", "").strip()
    if mode == "object-storage" and provider == "s3" and not region:
        raise ValueError("AGENTYARD_OBJECT_STORAGE_REGION is required when object storage provider is s3")

    access_key = os.getenv("AGENTYARD_OBJECT_STORAGE_ACCESS_KEY", "").strip()
    secret_key = os.getenv("AGENTYARD_OBJECT_STORAGE_SECRET_KEY", "").strip()
    bucket = os.getenv("AGENTYARD_OBJECT_STORAGE_BUCKET", "agentyard-knowledge").strip()
    if mode == "object-storage" and (not access_key or not secret_key):
        raise ValueError(
            "AGENTYARD_OBJECT_STORAGE_ACCESS_KEY and AGENTYARD_OBJECT_STORAGE_SECRET_KEY are required when object storage is enabled"
        )
    if mode == "object-storage" and not bucket:
        raise ValueError("AGENTYARD_OBJECT_STORAGE_BUCKET is required when object storage is enabled")
    ...
    return ObjectStorageSettings(
        ...
        create_bucket=_env_bool("AGENTYARD_OBJECT_STORAGE_CREATE_BUCKET", default_create_bucket),
        bucket=bucket,
    )
