...
const knowledgeDocumentTitles = computed(() =>
  props.configuration.knowledgeBase?.documents?.map((document) => document.title).join(' / ') || '-',
);
const knowledgeDocumentCount = computed(() => props.configuration.knowledgeBase?.documents?.length ?? 0);
...
<template>
  <a-descriptions v-if="resourceType === 'KNOWLEDGE_BASE' && configuration.knowledgeBase" :column="2" size="small">
    ...
    <a-descriptions-item label="文档规模">{{ knowledgeDocumentCount }}</a-descriptions-item>
    <a-descriptions-item label="导入文档标题" :span="2">{{ knowledgeDocumentTitles }}</a-descriptions-item>
  </a-descriptions>
