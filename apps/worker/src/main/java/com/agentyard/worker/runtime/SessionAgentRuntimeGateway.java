...
import com.agentyard.contracts.session.SessionContracts.FinalOutcomePayload;
...
public interface SessionAgentRuntimeGateway {
    ...
    class HttpSessionAgentRuntimeGateway implements SessionAgentRuntimeGateway {
        ...
        public HttpSessionAgentRuntimeGateway(
            @Value("${agentyard.agent-runtime.base-url}") String agentRuntimeBaseUrl,
            @Value("${agentyard.api.base-url:http://127.0.0.1:8080/api}") String apiBaseUrl,
            @Value("${agentyard.internal-auth.token}") String internalAuthToken,
            ObjectMapper objectMapper,
            MeterRegistry meterRegistry,
            @Value("${agentyard.agent-runtime.stream-idle-timeout:PT30S}") Duration streamIdleTimeout
        ) {
            ...
            this.streamStallCounter = Counter.builder("agentyard.runtime_stream.stream_stall")
                .description("Number of agent-runtime streaming stalls observed by the worker")
                .register(meterRegistry);
            this.missingFinalOutcomeCounter = Counter.builder("agentyard.runtime_stream.missing_final_outcome")
                .description("Number of agent-runtime streams that ended without FINAL_OUTCOME")
                .register(meterRegistry);
            this.streamRelayFailureCounter = Counter.builder("agentyard.runtime_stream.relay_failure")
