...
public class AssistantRunActivitiesImpl implements AssistantRunActivities {
    private static final Logger log = LoggerFactory.getLogger(AssistantRunActivitiesImpl.class);
    ...
    public WorkflowResult startExecution(WorkflowStartRequest request) {
        try (WorkerLogContext.Scope ignored = WorkerLogContext.open(request.logContext())) {
            log.info("assistant activity startExecution workflowId={} sessionId={}", request.workflowInstanceId(), request.sessionContext().sessionId());
    ...
    public WorkflowResult resumeExecution(WorkflowResumeRequest request) {
        try (WorkerLogContext.Scope ignored = WorkerLogContext.open(request.logContext())) {
            log.info("assistant activity resumeExecution workflowId={} sessionId={}", request.workflowInstanceId(), request.sessionContext().sessionId());
